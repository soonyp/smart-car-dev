...
//        }
        ...
        newThres = (uint8_t)((averageValue_LessThres * ImgConf.white_threshold + averageValue_MoreThres * (1.0f-ImgConf.white_threshold)));
    }while(_abs(newThres - lastThres) > deltThres);

    return _limit(newThres, low, top);
}

static uint8 Automatic_Threshold_new(uint8 *ImageCmprsData, uint8 row, uint8 column, uint8 *threshold, uint8 deltThreshold)
{
    ...
    do{
        ...
        newThreshold = (int)(averageValue_LessThreshold*ImgConf.white_threshold + averageValue_MoreThreshold*(1.0f-ImgConf.white_threshold));
    }
...
int16 CDM_MainStreet_Point(ConDomainNode *cdm, MainStreetType mst, _POINT_ *point)
{
    ...
    for (i = cdm->lastSign_equival; i > 0; --i)
    {
        if (cdm->equival[i] && cdm->equival[i - 1])
        {
            ...
            {
                if (cdm->enRun[i] - cdm->stRun[i] > MAIN_STREET_MIN_LEN && cdm->enRun[i - 1] - cdm->stRun[i - 1] > MAIN_STREET_MIN_LEN)
                {
