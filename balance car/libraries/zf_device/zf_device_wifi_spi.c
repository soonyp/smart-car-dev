...
static void wifi_spi_send_data_multi(wifi_spi_send_multi_struct *multi_buffer) {
    ...
    for (i = 0; i < WIFI_SPI_MAX_MULTI; i++) {
        if (NULL != multi_buffer->source[i])
            spi_write_8bit_array(WIFI_SPI_INDEX, multi_buffer->source[i], multi_buffer->length[i]);
    }
    gpio_set_level(WIFI_SPI_CS_PIN, 1);
    wifi_spi_send_done();
}
...
uint32 wifi_spi_write_data_multi(wifi_spi_send_multi_struct *multi_buffer) {
    ...
    // 记录需要发送的长度
    send_length =
            multi_buffer->length[0] + multi_buffer->length[1] + multi_buffer->length[2] + multi_buffer->length[3] +
            multi_buffer->length[4] + multi_buffer->length[5] + multi_buffer->length[6] + multi_buffer->length[7];
...
uint8 wifi_spi_init(char *wifi_ssid, char *pass_word, wifi_spi_mode_enum wifi_mode) {
    ...
    spi_init(WIFI_SPI_INDEX, SPI_MODE0, WIFI_SPI_SPEED, WIFI_SPI_SCK_PIN, WIFI_SPI_MOSI_PIN, WIFI_SPI_MISO_PIN,
             SPI_CS_NULL);//硬件SPI初始化
