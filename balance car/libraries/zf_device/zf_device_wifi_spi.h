...
uint8
wifi_spi_disconnected_wifi(void);                                                                      // 断开 WIFI 连接
uint8 wifi_spi_entry_serianet(void);                                                                      // 打开透传模式
uint8 wifi_spi_exit_serianet(void);                                                                      // 关闭透传模式

uint8
wifi_spi_connect_tcp_servers(char *ip, char *port, wifi_spi_transfer_mode_enum mode);                    // 建立 TCP 连接
uint8
wifi_spi_connect_udp_client(char *ip, char *port, char *local_port, wifi_spi_transfer_mode_enum mode);  // 建立 UDP 传输
uint8 wifi_spi_disconnect_link(
        void);                                                                      // 断开连接 TCP Server 使用本接口将会断开所有连接


uint32 wifi_spi_send_byte(uint8 data);                                                                // WIFI 模块发送字节函数
uint32
wifi_spi_send_buffer(const uint8 *buff, uint32 length);                                          // WIFI 模块发送缓冲区函数
uint32 wifi_spi_send_buffer_multi(
        wifi_spi_send_multi_struct *multi_buffer);                                  // WIFI 模块发送多缓冲区函数
uint32
wifi_spi_send_string(const char *str);                                                           // WIFI 模块发送字符串函数
